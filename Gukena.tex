\chapter{Gukena}
\label{Gukena}
\cambiar{El título del capítulo debería ser más descriptivo.}
\cambiar{Debe enmarcar el software en el uso de tecnologías según la Fig. 2.2. No se hace énfasis en qué etapa del proceso electoral se aplica Gukena.}
\cambiar{Se enumeran las ventajas del sistema antes de conocerlo. A su vez, cómo se corroboraron esas ventajas? Parecen parte de una conclusión o de un resultado de experiencias.}
\cambiar{}
...
\section{Análisis}
\cambiar{Sección 4.1. Si se nombra que se usa SCRUM, hay que explicarlo.
	○	Se nombran las historias de usuario, pero estas no están definidas como tales.
Puesto que las historias de usuarios se definen como:
Como [perfil], [quiero] [para]
■ [perfil] es para quien desarrollamos esto. Por ejemplo: como USUARIO, quiero…. para….
■ [quiero] es la intención que tenemos, no las funciones. Ejemplo: como USUARIO quiero ORGANIZAR LOS VOTOS, para…
■ [para] es el problema que quiero resolver. Ejemplo: Ejemplo: como USUARIO quiero ORGANIZAR LOS VOTOS, para MEJORAR LA CARGA}
\subsection{Metodología de desarrollo}
\cambiar{Es muy simple y resumida la explicación de la metodología de desarrollo utilizada. Además, ¿quién hizo Gukena? solo la tesista? no queda claro. ¿Cuánto duró su desarrollo? ¿En qué partes participó la tesista?}
...
\cambiar{Explica las historias, pero que son las memorias? Sección 4.1.3 no se termina de entender a qué apunta la sección.}
\cambiar{Es un sistema que funciona, pero no están bien explicadas sus partes.}
\cambiar{Lo que se nombra en la Sección 4.1.2 son más parecidas a las tarjetas Kanban}
...
\end{itemize}
\cambiar{La Figura 4.4 es ilegible y no está explicada.}
\begin{figure}[h!]
...
\cambiar{La sección 4.2.4 esta confusa. Tanto interoperabilidad y seguridad se deberían haber explicado en capítulos anteriores y aquí mostrado su aplicación y verificación para Gukena. Sino mezcla el concepto (Tabla 4.2 por ejemplo) al mismo tiempo que explica cómo lo hace para Gukena.}
\cambiar{Utiliza terminología en inglés que no se explica (Loguean, log, loguin, back, etc)}
\cambiar{Hay artículos publicados de este software?}
\cambiar{Hablan de LAPP y LAMP, ¿por qué motivo? no están explicados tampoco porque los citan y porque los utilizan.}
...
\cambiar{Sección 4.2.5, ¿por qué es parte del diseño del software? ¿no sería el proceso que termina en un diseño?}
\cambiar{Dice imagen en lugar de figura.}
...
\section{Desarrollo}
\cambiar{Recién en la Sección 4.3 conocemos el equipo de trabajo. Solo hicieron la implementación? y el análisis y diseño quién lo hizo? A su vez, como es una tesis de la estudiante Silvia Soto se debe conocer específicamente el trabajo que realizó la tesista.}
\cambiar{Se vuelve a especificar en la sección 4.3 el ciclo de vida y uso de SCRUM, ya dicho mucho antes. Se debería separar en secciones para conocer los trabajos de la tesista y los trabajos previos. ¿Qué trabajo quiere presentar la tesista en esta tesis?}
\cambiar{Siguen en la Sección 4.3 y describen MVC, pero en la Sección 4.2 ya describieron el diseño y mostraron una arquitectura diferente. Al mismo tiempo la explica. La figura 4.9 está en inglés, posiblemente copiada de alguna página web. Tiene números que no están descritos en el texto y no describen qué posee Gukena en cada componente.}
...
\cambiar{La figura 4.8 y 4.9 no son necesarias si no están bien explicadas}
\begin{figure}[h!]
    \begin{center}
...
\cambiar{Falta una conclusión del capítulo para cerrar el tema.}